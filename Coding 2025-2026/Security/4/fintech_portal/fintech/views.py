...
def account_view(request, account_id:int):
    ...
    if not (is_admin_user(request.user) or is_teller(request.user) or acc.owner==request.user): print(f"ACCESS DENIED: user={request.user}, account={acc.id}"); return HttpResponseForbidden("Access denied")
...
def download_statement_vuln(request, statement_id:int):
    ...
    allowed = is_admin_user(request.user) or st.account.owner==request.user or is_teller(request.user)
    if not allowed: print(f"ACCESS DENIED: user={request.user}, statement={st.id}, account={st.account.id}"); return HttpResponseForbidden("Access denied")
...
def export_user_profile(request, user_id:int):
    ...
    if request.user != u and not is_admin_user(request.user): print(f"ACCESS DENIED: user={request.user}, target_user={u.id}"); return HttpResponseForbidden("Access denied")
