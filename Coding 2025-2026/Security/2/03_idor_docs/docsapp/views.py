...
# @login_required
# @require_POST
# def document_update_secure(request, obj_id):
#     obj = get_object_or_404(Document, id=obj_id, owner=request.user)
...
# @login_required
# @require_POST
# def fileentry_update_secure(request, obj_id):
#     obj = get_object_or_404(FileEntry, id=obj_id, owner=request.user)
...
def index(request):
    my_lists = [
        ("/secure/document/list/","Document: мои объекты"),
        ("/secure/fileentry/list/","FileEntry: мои объекты")
    ]
    return render(request, "index.html", {"my_lists": my_lists, "domain_desc": "Документы и файлы"})
...
def document_detail_secure(request, obj_id):
    obj = get_object_or_404(Document, id=obj_id, owner=request.user)
    return render(request, "docsapp/document_detail.html", {"obj": obj})
...
def document_update_secure(request, obj_id):
    ...
    if 'title' in request.POST:
        obj.title = request.POST['title']
        obj.save()
    return redirect("document_detail_secure", obj_id=obj.id)
...
def fileentry_detail_secure(request, obj_id):
    obj = get_object_or_404(FileEntry, id=obj_id, owner=request.user)
    return render(request, "docsapp/fileentry_detail.html", {"obj": obj})
...
def fileentry_update_secure(request, obj_id):
    ...
    if 'name' in request.POST:
        obj.name = request.POST['name']
        obj.save()
    return redirect("fileentry_detail_secure", obj_id=obj.id)
